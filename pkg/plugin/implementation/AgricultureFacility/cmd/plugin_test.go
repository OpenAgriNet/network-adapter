...
func TestParseConfig(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			config:   map[string]string{},
			expected: &AgricultureFacility.Config{AuthByProvider: map[string]*common.AuthProfile{}},
		},
		{
			// Every key this plugin reads. Auth is per provider, in settings
			// named for the participant id, which pkg/plugin flattens from the
			// nested block an operator writes.
			...
			expected: &AgricultureFacility.Config{
				...
				AuthByProvider: map[string]*common.AuthProfile{
					"pocra": {Provider: "pocra", Scheme: "none"},
				},
				MaxResponseBytes:  2048,
				SearchConcurrency: 4,
			},
...
func TestNewBuildsAStep(t *testing.T) {
	...
	step, closer, err := agriFacilityProvider{}.New(context.Background(), stubRegistry{}, stubMapper{},
		map[string]string{
			"bindingKeys": "pocra|openagrinet:AgricultureFacility",
			// A served provider declares its own auth, even when it is none.
			"authScheme-pocra": "none",
		})
	if err != nil {
