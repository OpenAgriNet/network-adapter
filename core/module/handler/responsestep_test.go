...
func TestSendNack(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "coded sign validation (401)",
			err:      model.NewSignValidationErr("AUT_SUBSCRIBER_NOT_FOUND", errors.New("no subscriber found with given credentials")),
			status:   http.StatusUnauthorized,
	...
	v2Tests := []struct {
		...
		{
			name:     "v2 sign validation (401)",
			err:      model.NewSignValidationErr("AUT_SIGNATURE_INVALID", errors.New("signature expired")),
			status:   http.StatusUnauthorized,
		...
		{
			name:     "v2 coded sign validation key expired (401)",
			err:      model.NewSignValidationErr("AUT_KEY_EXPIRED_OR_REVOKED", errors.New("subscriber key is expired or revoked")),
			status:   http.StatusUnauthorized,
