...
// stubProviderMapper and stubProviderRegistry satisfy the two dependencies
// loadProviderStep requires before it will build a provider step. The registry
// implements ProviderRecordLookup as well, which loadProviderStep narrows to.
type stubProviderMapper struct{}
...
func (stubProviderRegistry) Lookup(context.Context, *model.Subscription) ([]model.Subscription, error) {
	return nil, nil
}

func (stubProviderRegistry) ProviderRecord(context.Context, string) (*model.ProviderRecord, error) {
	return nil, nil
}
...
func TestInitStepsRefusesTwoProviderStepsWithTheSameID(t *testing.T) {
	// The mapper and registry are not decoration. loadProviderStep checks for
	// both up front, so without them startup fails on the FIRST entry with
	// "Mapper plugin not configured" and the loop never reaches the second --
	// meaning the duplicate guard never runs. This test passed with the guard
	// deleted outright, because that error also contains the id, which was all
	// the assertion checked.
	h := &stdHandler{
		moduleName: "test-module",
		mapper:     &stubProviderMapper{},
	...
	// Asserted on the duplicate wording, not just the id: the id appears in
	// every loadProviderStep failure too, so matching it alone cannot tell the
	// duplicate refusal from a missing dependency.
	if !strings.Contains(err.Error(), "configured more than once") {
		t.Errorf("error %q should be the duplicate-id refusal", err)
	}
...
// A single entry must still load, or the test above would pass on any failure
// at all rather than specifically on the duplicate.
func TestInitStepsAcceptsOneProviderStep(t *testing.T) {
	h := &stdHandler{
		moduleName: "test-module",
		mapper:     &stubProviderMapper{},
		registry:   &stubProviderRegistry{},
	...
	cfg := &Config{
		Plugins: PluginCfg{
			ProviderSteps: []plugin.Config{{ID: "mausamgram", Config: map[string]string{}}},
		},
	}

	if err := h.initSteps(context.Background(), noopPluginManager{}, cfg); err != nil {
		t.Fatalf("one provider step should load: %v", err)
	}
	if !h.hasProviderSteps {
		t.Error("hasProviderSteps should be set once a provider step is configured")
	}
}
...
// A step's answer has to be an envelope. A mapping whose response half is
// written as `$.response.temperature` rather than as an object produces `28.5`,
// which is valid JSON -- so Content-Type was not a lie -- and the adapter
// answered 200 with it and then SIGNED it. A consumer looking for
// message.contract finds nothing and cannot tell that from a protocol change.
//
// Refused rather than passed on, because a signed confident non-answer is worse
// than a NACK: the caller cannot retry what it does not know failed, and the
// signature says this adapter meant it.
//
// Driven through ServeHTTP with a real ack signer installed, which is the whole
// point. The check used to sit in sendResponse, after the response steps -- so
// it refused an answer the signer had already covered, and shipped a NACK body
// under a Signature over the scalar. Testing sendResponse directly could not
// see that, because the signer is not in that call.
func TestServeHTTPDoesNotSignAnAnswerItRefuses(t *testing.T) {
	signer := &mockSigner{returnSig: "sig"}
	as, err := newAckSignerStep(signer, &mockKM{
		keyset: &model.Keyset{UniqueKeyID: "k1", SigningPrivate: "priv"}})
	if err != nil {
		t.Fatalf("failed to build the ack signer: %v", err)
	}
	// Asserted to the concrete type exactly as initSteps does, so h.ackSigner
	// is populated and the NACK path can sign -- which is what is under test.
	ackSigner, ok := as.(*ackSignerStep)
	if !ok {
		t.Fatalf("newAckSignerStep returned %T, want *ackSignerStep", as)
	}
	...
	h := &stdHandler{
		...
		role:             model.RoleBPP,
		moduleName:       "test-module",
		steps:            []definition.Step{&answeringStep{answer: []byte(`28.5`)}},
		responseSteps:    []definition.ResponseStep{as},
		ackSigner:        ackSigner,
		hasProviderSteps: true,
	}
	...
	// The scalar must not reach the wire, and the caller must be told.
	if recorder.Code == http.StatusOK {
		t.Errorf("status = %d; a body that cannot carry a message must not be a 200", recorder.Code)
	}
	if strings.Contains(recorder.Body.String(), "28.5") {
		t.Error("the scalar was written to the wire")
	}
	var got model.Response
	if err := json.Unmarshal(recorder.Body.Bytes(), &got); err != nil {
		t.Fatalf("failed to decode the response: %v", err)
	}
	if got.Message.Status != model.StatusNACK {
		t.Errorf("status = %q, want a NACK", got.Message.Status)
	}

	// And the part that was actually broken: the signature that ships must
	// cover the body that ships. Before this fix it covered the refused
	// scalar while a NACK went out beneath it, and a peer verifying that sees
	// a digest mismatch -- reading a mapping bug as suspected tampering.
	//
	// Asserted on the bytes signed, not on the header text: mockSigner returns
	// a fixed signature string whatever it is given, so the header alone
	// cannot tell the two bodies apart.
	if string(signer.signedBody) == "28.5" {
		t.Error("the refused scalar was signed; the refusal has to happen before the signer runs")
	}
	if signer.signAckCalled && string(signer.signedBody) != recorder.Body.String() {
		t.Errorf("signed %q but sent %q -- the signature must cover what ships",
			signer.signedBody, recorder.Body.String())
	}
}

// The ordinary path must be untouched: a real envelope is answered, signed, and
// sent, so the guard above cannot be satisfied by refusing everything.
func TestServeHTTPStillSignsAndSendsARealEnvelope(t *testing.T) {
	const answer = `{"context":{"action":"on_select"},"message":{"contract":{}}}`
	...
	h := &stdHandler{
		...
		role:             model.RoleBPP,
		moduleName:       "test-module",
		steps:            []definition.Step{&answeringStep{answer: []byte(answer)}},
		responseSteps:    []definition.ResponseStep{as},
		ackSigner:        ackSigner,
		hasProviderSteps: true,
	}
	...
	if recorder.Body.String() != answer {
		t.Errorf("body = %s, want the answer unchanged", recorder.Body.String())
	}
	if string(signer.signedBody) != answer {
		t.Errorf("signed %q, want the answer that was sent", signer.signedBody)
	}
	if sig := recorder.Header().Get("Signature"); !strings.Contains(sig, "sig-over-the-answer") {
		t.Errorf("Signature header = %q, want the answer's signature", sig)
	}
}
